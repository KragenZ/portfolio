...
document.querySelectorAll('.btn').forEach(btn => {
    btn.addEventListener('mousemove', (e) => {
        ...
        gsap.to(btn, { x: pullX, y: pullY, ease: "power2.out", duration: 0.3 });
    });
    btn.addEventListener('mouseleave', () => {
        gsap.to(btn, { x: 0, y: 0, ease: "elastic.out(1, 0.3)", duration: 1.2 });
    });
});

// --- Neural Spine Scroll Tracker ---
gsap.to('#neural-spine', {
    height: "100%",
    ease: "none",
    scrollTrigger: {
        trigger: "body",
...
// --- Shery.js WebGL Image Distortions ---
